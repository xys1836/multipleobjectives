...
TypeOfVNFs = ['VNF' + str(i+1) for i in range(NbOfVNFTypes)]
...
def generate_SFCRequest(NbOfSFCs,
    ...
    for i in range(1, NbOfSFCs + 1):
        ...
        src = random.randint(1, NbOfNodes)
        dst = random.randint(1, NbOfNodes)
        while(dst == src):
            dst = random.randint(1, NbOfNodes)
        latency = MaxNumber
        bw = 0
        lengthOfSFC = random.randint(MinLengthOfSFC, MaxLengthOfSFC)
        if(LatencySensitive):
            latency = random.randint(LatencyLow, LatencyHigh) #* lengthOfSFC
        if(BwSensitive):
        ...
        for vnf in vnfs:
            VNFList = VNFList + "\"" + vnf + "\", "
            c.append(random.randint(CPURequirementLow, CPURequirementHigh))
        VNFList = VNFList + "}"
        sfc_c.append(c)
...
print VNFOpCost
print SFCRequest
print 'sfc_c = ' + str(sfc_c) + ';'

#
#
# print generate_VNFOpCost(TypeOfVNFs, NbOfWorkNodes, NbOfNodes, 1, 10)
#
VNFOpCost = "VNFOpCost =#[\n"
...
for t in ['VNF1']:
    vnf = "\"" + t + "\": " + "#["
    for i in range(1, NbOfWorkNodes + 1):
        vnf = vnf + str(i) + ": " + str(random.randint(OpCostLow, OpCostHigh)) + ", "
    for i in range(NbOfWorkNodes + 1, NbOfNodes + 1):
        vnf = vnf + str(i) + ": " + str(0) + ", "
    VNFOpCost = VNFOpCost + vnf + "]#,\n"
VNFOpCost = VNFOpCost + "]#;\n"
