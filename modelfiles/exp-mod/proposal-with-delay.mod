 ...
 * OPL 12.9.0.0 Model
 * Author: Yansen XU
 * Creation Date: Jan 29, 2020 at 11:37:56 AM
 *********************************************/
/*********************************************
 * OPL 12.8.0.0 Model
 * Author: yansenxu
...
int VNFOpCostPreWork[TypeOfVNFs][NodeSet] = ...;
int VNFSetupCostPreWork[TypeOfVNFs] = ...;
/*
...
minimize 
  sum(t in TypeOfVNFs)
    ...
    sum(r in SFCRequests)
    	sum(i in 1..card(r.VNFList))
    	  sum(v in NodeSet)
      		y[r][i][v] * sfc_c[r][i] * VNFOpCostPreWork[item(r.VNFList, i-1)][v] ;
...
execute {
	...
	for(var r in SFCRequests){
      ...
      for(var t in TypeOfVNFs){
      	for(var v in NodeSet)      
      		VC = VC + x[t][v] * VNFSetupCostPreWork[t];
      }
      writeln("SET UP Cost prework para: ", VC);
      OC = 0;
      for(var r in SFCRequests){
      var count = 0;      
      	...
      	for(var vnf in r.VNFList){
			count = count + 1;		
			for(var v in NodeSet){
				OC = OC + 	y[r][count][v] * sfc_c[r][count] * VNFOpCostPreWork[vnf][v] ;
			}	
		}	
      }
      writeln("OP Cost prework para: ", OC);
};
