 ...
 * OPL 12.8.0.0 Model
 * Author: yansenxu
 * Creation Date: May 7, 2018 at 7:51:42 PM
 *********************************************/
 /*********************************************
 * OPL 12.8.0.0 Model
 * Author: yansenxu
...
//  					 x[item(sfcRequest.VNFList, i-1)][v] == 0 => y[sfcRequest][i][v]== 0;		
  	...
  	forall(e in Edges){
  		sum(sfcRequest in SFCRequests)
  	  		z[sfcRequest][<e.u, e.v>] * sfcRequest.bandwidth
  	  	+
  	  	sum(sfcRequest in SFCRequests)
  	  		z[sfcRequest][<e.v, e.u>] * sfcRequest.bandwidth
  	    <= BandwidthCapacity[<e.u, e.v>];
  	}
	...
	// for keeping sfc order
  	forall(sfcRequest in SFCRequests){
  		forall(i in 1..(card(sfcRequest.VNFList)-1)){
  		 	forall(v in NodeSet: v != sfcRequest.src && v != sfcRequest.dst){
  				forall(u in NodeSet:   u != sfcRequest.src && u != v){
  	...
  	forall(sfcRequest in SFCRequests){
  		forall(i in 1..(card(sfcRequest.VNFList)-1)){
  			...
  			forall(u in NodeSet){
  					y[sfcRequest][card(sfcRequest.VNFList)][u] == 1 =>
	  				u == sfcRequest.dst;
  			}		
  		}  	  	
  	}
...
//		var used = 0;
//		for(var r in SFCRequests){
//			used = used + z[r][e] * r.bandwidth;		
//		}
//		write(": ", used);
//		writeln("/ ", BandwidthCapacity[e]);
//	}	
//	writeln("/--- ");
	...
	for(var e in Edges){
		...
		for(var r2 in SFCRequests){
			for(var e2 in Edges){
				if(e2.u == e.v && e2.v == e.u){			
					used = used + z[r2][e2] * r2.bandwidth;
    			}						
			}
 		}
		write(": ", used);
	...
	for(var v in NodeSet) {
		...
		for(var r in SFCRequests){
			...
			for(var vnf in r.VNFList){
				count = count + 1;		
				used = used + y[r][count][v] * sfc_c[r][count];
			}		
		}
		write(": ", used);
		writeln("/ ", CpuCapacity[v]);
	}
	...
	for(var r in SFCRequests){
		...
		var numberOfLinks = count;
		write(" ", numberOfLinks);	
		var latency = 0;
		for(var e in Edges){
			if(z[r][e] == 1) {
				latency = latency + Latency[e];			
					
				write(" ", e);
  			}					
		}
		write(" Latency edge: ", latency);
		writeln("");
    }	
	...
	for(var r in SFCRequests){
      ...
      var VC = 0;
      for(var t in TypeOfVNFs){
      	for(var v in NodeSet)      
      		VC = VC + x[t][v] * VNFSetupCost[t];
      }
      writeln("SET UP Cost: ", VC);
      var OC = 0;
      for(var r in SFCRequests){
      var count = 0;      
      	...
      	for(var vnf in r.VNFList){
			count = count + 1;		
			for(var v in NodeSet){
				OC = OC + 	y[r][count][v] * sfc_c[r][count] * VNFOpCost[vnf][v] ;
			}	
		}	
      }
      writeln("OP Cost: ", OC);
